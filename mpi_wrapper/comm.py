...
class Communicator(object):
    ...
    def myAllreduce(self, src_array, dest_array, op=MPI.SUM):
        ...
        if rank == 0:
            np.copyto(dest_array, src_array) #initialize dest_array 
    ...
    def myAlltoall(self, src_array, dest_array):
        ...
        temp_recv = np.empty(seg_size,dtype=src_array.dtype)
        
        for i in range(size):
            offset = i * seg_size
            
            if i == rank: # direct copy
                dest_array[offset:offset+seg_size] = src_array[offset:offset+seg_size]
            else:
                self.comm.Sendrecv(
                    sendbuf = src_array[offset:offset+seg_size],
                    dest = i, # send to other rank
                    recvbuf = temp_recv,
                    source = i # receive from other rank
                )
                dest_array[offset:offset+seg_size] = temp_recv
